...
small_fn_vec = vcat(reverse([string("-", i) for i in 1:25]), "0", [string("+", i) for i in 1:25])
ϕ_vec = zeros(length(small_fn_vec))
ϕ_vec_deg = zeros(length(small_fn_vec))

for (ind, small_fn) in pairs(small_fn_vec)
    cd("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv6/troughs2/troughs2_png/")


    raw_image = Images.Gray.(Images.load(string(small_fn, ".png")))[270:670, 700:1150]
    dat = Float64.(raw_image)

    correction = (Float64.(Images.Gray.(Images.load("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv6/background/backgroundv3.png"))[270:670, 700:1150])) .^ (-1)
...
cd("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv6/troughs2")
...
# raw_image = Images.Gray.(Images.load(string(fn, ".png")))[270:710, 700:1350]
# dat = Float64.(raw_image)

# correction = (Float64.(Images.Gray.(Images.load("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv6/background_low_intensity.png"))[270:710, 700:1350])) .^ (-1)
# correction = (*(size(correction)...) / sum(correction)) * correction;
# dat = dat .* correction
