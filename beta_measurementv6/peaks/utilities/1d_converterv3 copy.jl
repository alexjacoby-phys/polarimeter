...
index = DelimitedFiles.readdlm("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/peaks/exact_angles.txt")


cd("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/peaks/peaks_png/")
...
γ = 12.6 * (π / 180)
# begin
    raw_imagepr = Images.Gray.(Images.load(string("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/peaks/peaks_png/",small_fn,".png")));

    raw_image = OffsetArrays.centered(ImageTransformations.imrotate(raw_imagepr, γ))[-200:200, 20:600];
    dat = Float64.(raw_image)

    correction_imagepr = Images.Gray.(Images.load("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/background_low_intensity.png"))
    correction_image = OffsetArrays.centered(ImageTransformations.imrotate(correction_imagepr, γ))[-200:200, 20:600]
...
Plots.plot(1:partitions, params[:, 5], label ="Data", title = "Overall Phase");
Plots.plot!(1:partitions, linear_model(Vector(1:partitions), linear_fit.param), ylims = [0,2π], label = "Fitted")

Plots.plot(1:partitions, params[:, 6] .- π,ylabel = "Relative Phase Minus Pi", xlabel = "Partition Number", label = :none)


sum(params[:,6])/length(params[:,6])
Plots.savefig("Rel_Phase_Peaks.pdf")
...
Plots.gif(animation,  fps=100, "Strip_Fit_Peaks.gif")
Plots.gif
params
