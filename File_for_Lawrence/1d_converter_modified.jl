...
fn = string("1f_sample.png")

raw_image = Images.Gray.(Images.load(fn))[150:900, 300:1300];
...
dat =Float64.(raw_image)
# this sets the number of partitions for the one dimensional sample. Must be integer valued
...
LX = M
LY = N
((avgpoints1,dists1),(avgpoints2,dists2)) = avgpoints(no_partitions,θ; LX = Float64(M), LY = Float64(N))
...
single_slice = pts_in_avg_slice(pavg, normal=n2,A= 1, B= LX, C = 1, D = LY)

single_slicex = [pt[2] for pt in single_slice]
single_slicey = [pt[1] for pt in single_slice]


Plots.plot(raw_image);
Plots.plot!(single_slicex,single_slicey)


Plots.plot([itp[pt...] for pt in single_slice],ylims = (0,0.8),legend = :none)
Plots.savefig("single_slice.pdf")
DelimitedFiles.writedlm("1f_sample.txt",[itp[pt...] for pt in single_slice])
...
Plots.plot!(avgpoints1_x, avgpoints1_y,color = :red);
Plots.plot!(avgpoints2_x, avgpoints2_y, legend = :none, color = :red)
Plots.savefig("Avglines.pdf")
