...
index1 = DelimitedFiles.readdlm("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/troughs/Estimated_Angles.txt")
index2 = DelimitedFiles.readdlm("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/troughs/exact_angles.txt")
...
Plots.plot(index2[3, :], parms[:, 6]-(parms[:, 6] .> π) * 2π, yerror=error[:, 6], seriestype=:scatter, label="Relative Phase", title="Relative Phases for Trough Matched Point")
linear_model(x::Vector{Float64}, parms2::Vector{Float64}) = parms2[2] .+ parms2[1] * x
p0 = [0.0, 0]
linear_fit = LsqFit.curve_fit(linear_model, index2[3, :], parms[:, 6] - (parms[:, 6] .> π) * 2π, p0)
p = linear_fit.param
-p[2] / p[1]
Plots.plot!(index2[3,:], linear_model(index2[3,:], p), linewidth = 4, label = "Fitted")


findmin(abs.(parms[:, 6] - (parms[:, 6] .> π) * 2π))
