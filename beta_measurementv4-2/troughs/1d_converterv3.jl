...
index = DelimitedFiles.readdlm("/Users/alexjacoby/Documents/Research_Code/polarimeter/beta_measurementv4-2/troughs/exact_angles.txt")
...
Plots.plot(1:partitions, params[:, 6] - (params[:, 6] .> π) * 2π, ylabel="Relative Phase (Radians)", xlabel = "Partition Number", label = :none)
Plots.savefig("Rel_Phase_Troughs.pdf")


sum(params[:, 6] - 2π*(params[:, 6] .> π)) / length(params[:, 6])
...
Plots.gif(animation, fps=100, "Strip_Fit_Troughs.gif")
